...
export const LaunchExpensesDialog = ({
  ...
  const handleSubmit = async () => {
    try {
      ...
      const request: BulkGenerateRequest = {
        month: selectedMonth,
        expense_values: fixedExpenses.map((exp) => ({
  ...
  const totalValue = Object.values(expenseValues).reduce((sum, val) => sum + val, 0);
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl max-h-[90vh]">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <Label>Despesas ({fixedExpenses.length})</Label>
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button
              ...
              disabled={isSubmitting || fixedExpenses.length === 0}
            >
              {isSubmitting ? 'Gerando...' : `Gerar ${fixedExpenses.length} Despesas`}
            </Button>
          </div>
        </div>
      </DialogContent>
